...
class HarvesterAPI:
    ...
    async def new_signage_point_harvester(
        ...
        def blocking_lookup(filename: Path, plot_info: PlotInfo) -> List[Tuple[bytes32, ProofOfSpace, bool]]:
            ...
            try:
                ...
                responses: List[Tuple[bytes32, ProofOfSpace, bool]] = []
                if quality_strings is not None:
                    ...
                    for index, quality_str in enumerate(quality_strings):
                        ...
                        plot_public_key = ProofOfSpace.generate_plot_public_key(local_sk.get_g1(), farmer_public_key)
                        responses.append(
                            (
                                quality_str,
                                ProofOfSpace(
        ...
        async def lookup_challenge(filename: Path, plot_info: PlotInfo) -> List[harvester_protocol.NewProofOfSpace]:
            ...
            all_responses: List[
                Union[
                    ...
                    harvester_protocol.NewPartialProofOfSpace,
                ]
            ] = []
            if self.harvester._is_shutdown:
                return []
            proofs_of_space_and_q: List[Tuple[bytes32, ProofOfSpace, bool]] = await loop.run_in_executor(
                self.harvester.executor, blocking_lookup, filename, plot_info
            )
            for quality_str, proof_of_space, partial in proofs_of_space_and_q:
                if partial:
                    if self.harvester.server.old_harvester:
                        continue
        ...
        for sublist_awaitable in asyncio.as_completed(awaitables):
            for response in await sublist_awaitable:
                ...
                if isinstance(response, harvester_protocol.NewProofOfSpace):
                    msg = make_msg(ProtocolMessageTypes.new_proof_of_space, response)
                else:
                    msg = make_msg(ProtocolMessageTypes.new_proof_of_space_partial, response)
        ...
        self.harvester._state_changed("farming_info", farming_info)
