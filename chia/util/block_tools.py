...
class BlockTools:
    ...
    def __init__(
        ...
        const_dict=None,
        farmer_entropy=b"block_tools farmer key",
        pool_entropy=b"block_tools pool key",
        counter_start=0,
        plot_count=20,
    ):
        self._tempdir = None
        ...
        self.keychain = Keychain(f"testing-1.8.0_{pool_entropy}", True)
        self.keychain.delete_all_keys()
        self.farmer_master_sk_entropy = std_hash(farmer_entropy)
        self.pool_master_sk_entropy = std_hash(pool_entropy)
        ...
        self._config["pool_public_key"] = bytes(self.pool_pk).hex()
        self._config["farmer_public_key"] = bytes(self.farmer_pk).hex()
        ...
        self.init_plots(root_path, counter_start, plot_count)
    ...
    def init_plots(self, root_path: Path, counter_start=0, plot_count=20):
        plot_dir = get_plot_dir(counter_start)
        mkdir(plot_dir)
        temp_dir = plot_dir / "tmp"
        mkdir(temp_dir)
        num_pool_public_key_plots = plot_count
        args = Namespace()
        ...
        test_private_keys = [
            ...
            for i in range(counter_start, counter_start + num_pool_public_key_plots)
        ]
        try:
...
def get_plot_dir(counter_start):
    cache_path = (
        Path(os.path.expanduser(os.getenv("CHIA_ROOT", "~/.chia/bluepool"))) / f"test-plots/offset_{counter_start}"
    )
    mkdir(cache_path)
    return cache_path
