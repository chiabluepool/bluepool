...
class FullNode:
            ...
            for height in range(start_height, target_height, batch_size):
                ...
                async with self.blockchain.lock:
                    (
                        ...
                        advanced_peak,
                        fork_height,
                    ) = await self.receive_block_batch(response.blocks, peer, None)
                    if not success:
            ...
            if peak_sync_hash is not None and request.header_hash != peak_sync_hash and peak_sync_height is not None:
                ...
                if peer.peer_node_id not in peak_peers:
                    target_peak_response: Optional[RespondBlock] = await peer.request_block(
                        full_node_protocol.RequestBlock(uint32(peak_sync_height), False),
                        timeout=10,
                    )
        ...
        try:
            ...
            for peer in self.server.all_connections.values():
                if peer.connection_type == NodeType.FULL_NODE:
                    ...
                    coroutines.append(
                        peer.request_block(
                            full_node_protocol.RequestBlock(uint32(heaviest_peak_height), True),
                            timeout=10,
                        )
                    )
            for i, target_peak_response in enumerate(await asyncio.gather(*coroutines)):
                if target_peak_response is not None and isinstance(target_peak_response, RespondBlock):
                    self.sync_store.peer_has_block(
                        heaviest_peak_hash,
                        peers[i],
                        heaviest_peak_weight,
                        heaviest_peak_height,
                        False,
                    )
            ...
            try:
                (
                    validated,
                    fork_point,
                ) = await self.weight_proof_handler.validate_weight_proof(response.wp)
            except Exception as e:
                await weight_proof_peer.close(600)
    ...
    async def new_infusion_point_vdf(
        ...
        timelord_peer: Optional[ws.WSChiaConnection] = None,
    ) -> Optional[Message]:
            ...
            async with self.blockchain.lock:
                if self.mempool_manager.get_spendbundle(spend_name) is not None:
                    ...
                    return (
                        ...
                        Err.ALREADY_INCLUDING_TRANSACTION,
                    )
                ...
                for index, sub_slot in enumerate(block.finished_sub_slots):
                    if (
                        ...
                        new_proofs = dataclasses.replace(
                            sub_slot.proofs,
                            infused_challenge_chain_slot_proof=vdf_proof,
                        )
                        new_subslot = dataclasses.replace(sub_slot, proofs=new_proofs)
                        new_finished_subslots = block.finished_sub_slots
                        new_finished_subslots[index] = new_subslot
