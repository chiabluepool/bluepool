...
class ChiaServer:
    ...
    async def start_client(
        ...
        if self._local_type is NodeType.HARVESTER:
            username = self.node.username
            password = self.node.password
            if (username is None or password is None) and not self.old_harvester:
                return False
        else:
            username = None
            password = None
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSChiaConnection, task_id):
                ...
                try:
                    ...
                    await connection.close(
                        self.api_exception_ban_seconds,
                        WSCloseCode.PROTOCOL_ERROR,
                        Err.UNKNOWN,
                    )
                finally:
